...
Hệ thống POS và điều phối vận hành nhà hàng, gồm gọi món/gọi thêm theo bàn, hàng đợi bếp FIFO theo từng lượt gọi, thanh toán tổng hợp, phiếu bếp 80 mm, hóa đơn A4, báo cáo trong ngày và Dashboard quản trị. Dữ liệu nghiệp vụ được lưu tại MySQL; frontend không tự quyết định giá, tổng tiền hoặc ETA cuối cùng.
...
Trạng thái hiện tại: chạy được end-to-end, 16 unit test, typecheck, production build và smoke test qua MySQL đều đạt. Migration tự chuyển order cũ thành lượt gọi số 1, bổ sung nhân viên/danh mục lịch sử mà không xóa dữ liệu. Cấu trúc phù hợp cho ứng dụng nội bộ hoặc MVP; phần [Đề xuất trước khi lên production](#đề-xuất-trước-khi-lên-production) liệt kê các bước còn cần khi mở rộng.
...
- Quản lý bàn, món, danh mục, thời gian nấu, nhân viên/ca làm và cấu hình thương hiệu.
- Báo cáo aggregate từ hóa đơn đã trả: KPI, 24 giờ, phương thức, món bán chạy, danh mục và hiệu suất nhân viên.
...
5. Toàn bộ timestamp queue được lưu và xử lý theo UTC.
6. Giá, tùy chọn món, tổng thanh toán và ETA đều được backend tính lại.
7. `/api/operations` đọc bàn/order/batch/config trong một repeatable-read snapshot duy nhất.
...
│        ├─ app/reporting.ts      # Fallback gom báo cáo đủ 24 giờ
...
|---|---|
| Bàn `101–108` | Bàn trống, đặt trước, đang chờ và đã xong |
...
| `active_orders` | Giỏ hàng tổng hợp đang mở của mỗi bàn, dùng khi thanh toán | unique `table_id`, FK bàn; xóa order sẽ cascade các batch |
...
| `payment_transactions` | Header hóa đơn đã trả và snapshot nhân viên | unique `invoice_code`, index `paid_at/staff_id` |
...
Audit trực tiếp database ngày 14/07/2026 cho kết quả `0` ở cả sáu nhóm lỗi cũ: bàn active không có order, order gắn sai trạng thái bàn, JSON order lỗi, tổng payment sai công thức, payment thiếu item và lệch `item_count`. Audit sau migration batch cũng cho `0` order thiếu batch, batch mồ côi, batch gắn sai bàn, JSON batch lỗi, trạng thái bàn lệch batch và dữ liệu smoke còn sót.
...
5. Tất cả món từ mọi lượt gọi được ghi vào cùng header; item lưu snapshot danh mục để báo cáo lịch sử không đổi theo catalog.
6. Active order bị xóa, bàn trở về `empty` và queue chạy tiếp trong cùng transaction.
7. Chỉ sau khi commit thành công frontend mới cho in hóa đơn A4 tổng hợp.

### Báo cáo

- Frontend gửi biên ngày địa phương dưới dạng UTC và `timezoneOffsetMinutes` để bucket giờ đúng múi giờ thiết bị POS.
- API aggregate trực tiếp `payment_transactions/payment_items`; KPI không phụ thuộc danh sách giới hạn hoặc order đang mở.
...
| `GET` | `/api/operations` | Snapshot bàn, order tổng hợp, số lượt và queue batch |
...
| `GET` | `/api/reports/summary?from=&to=&timezoneOffsetMinutes=` | Aggregate KPI/giờ/phương thức/món/danh mục/nhân viên |
...
| `npm run db:seed:test` | Làm mới bộ bàn/order/batch/payment demo |
...
Smoke test pause bếp có khôi phục cấu hình, tạo bàn tạm, kiểm tra ETA theo quantity, sửa phiếu chờ vẫn giữ FIFO, chặn thao tác bếp bằng batchId cũ, xác nhận thanh toán sớm bị từ chối, gọi thêm thành batch riêng, kiểm tra snapshot tổng hợp rồi hủy order toàn-waiting và dọn dữ liệu.
...
| Main JS hiện tại sau lazy-load | 208,61 KB / 65,69 KB gzip |
| Chunk biểu đồ (chỉ tải khi mở Báo cáo) | 394,59 KB / 108,69 KB gzip |
| Main CSS trước dọn UI thừa | 104,58 KB / 18,55 KB gzip |
| Initial CSS responsive/modal | 32,73 KB / 7,97 KB gzip |
...
| `/api/reports/summary` payload / p50 / p95 | 2,08 KB / 3,09 ms / 3,77 ms (10 mẫu) |
...
Shell ứng dụng dùng toàn bộ `100vw`, không còn giới hạn `max-width: 1440px` gây hai dải trống trên màn hình rộng. Giao diện đã được đo bằng Chrome ở 320, 390, 768, 861, 1024 và 1366 px; login chuyển một cột từ 920 px, modal mobile phủ đủ `100dvh`, form quản trị wrap theo màn hình và không còn tràn ngang đã biết. Top bar chỉ dùng một logo ngang CAS lớn; màu thương hiệu/nền quản trị đủ phân tách; bottom navigation chuyển thành sidebar trên màn hình lớn.
...
- Modal quản lý bàn, tùy chỉnh món, giỏ hàng, thanh toán và xác nhận nguy hiểm được render bằng portal trực tiếp vào `body`, phủ đủ viewport, khóa focus và cuộn nội dung bên trong nên không bị đứt overlay ở màn hình thấp.
...
6. Lưu tên khách, số khách và ghi chú theo từng order; hiện các trường này còn lấy từ cấu hình chung khi in bill.
7. Mở rộng đặt bàn với tên/điện thoại/khung giờ/số khách, chống trùng lịch và tự giải phóng bàn.
