...
import { isKitchenOrderStale, processKitchenQueue, promoteKitchenQueue } from '../src/kitchenQueue.js';
...
test('chu kỳ bếp không tự hoàn tất batch theo ETA khi bếp tạm dừng', async () => {
  const calls = [];
  const connection = {
  ...
  assert.equal(calls.some(call => typeof call === 'string' && call.includes('TIMESTAMPADD')), false);
  assert.equal(calls.some(call => typeof call === 'string' && call.includes('UPDATE order_batches')), false);
