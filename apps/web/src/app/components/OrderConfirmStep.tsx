...
import { Table, CartItem, cartEstimatedCookMinutes, cartItemCookMinutes, cartItemTotal, cartTotal, formatVND, STATUS_CONFIG } from '../data';
...
export function OrderConfirmStep({ table, cart, isAddition, isEditing, onCartChange, onBack, onEdit, onPlaceOrder }: OrderConfirmStepProps) {
  ...
  const updateQty = (cartId: string, delta: number) => {
    onCartChange(cart.map(i =>
      i.cartId === cartId ? { ...i, quantity: Math.min(99, Math.max(1, i.quantity + delta)) } : i
    ));
  };
      ...
      <div style={{ padding: '14px 16px 24px', borderTop: '1px solid #F3F4F6', background: '#fff', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 14, padding: '15px', cursor: submitting ? 'wait' : 'pointer', fontWeight: 700,
